...
%             c
%             o
            meanSDF = mean(Data.(opt.epochArray{e}).(conditionArray{c}).(goOutcomeArray{o}).sdf);
            align = Data.(opt.epochArray{e}).alignGo;
            % shorten the plotted sdf if needed, based on RT
            if strcmp(opt.epochArray{o}, 'checkerOn') && median(Data.checkerOn.(conditionArray{c}).(goOutcomeArray{o}).rt)+align < length(meanSDF)
                meanSDF = meanSDF(1 : round(median(Data.checkerOn.(conditionArray{c}).(goOutcomeArray{o}).rt))+align);
           end
        ...
        for o = 1 : length(stopOutcomeArray)
            meanSDF = mean(Data.(opt.epochArray{e}).(conditionArray{c}).(stopOutcomeArray{o}).sdf);
            align = Data.(opt.epochArray{e}).alignStop;
            % shorten the plotted sdf if needed, based on RT
            if strcmp(opt.epochArray{o}, 'checkerOn') && median(Data.checkerOn.(conditionArray{c}).(stopOutcomeArray{o}).rt)+align < length(meanSDF)
                meanSDF = meanSDF(1 : round(nanmedian(Data.checkerOn(conditionArray{c}).(stopOutcomeArray{o}).rt))+align);
            end
